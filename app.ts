...
const normalizeData = (unnormalizedData: Array<Post>): NormalizedData => {
    let normalPosts: NormalizedData = {
        byId: {}, 
        allIds: []
    }

    for (let post of  unnormalizedData) {
        normalPosts.allIds.push(post.id)
        normalPosts.byId[post.id] = {
            id: post.id,
            title: post.title,
            body: post.body
        } as Post
